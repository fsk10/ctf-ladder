const DEFAULT_SETTINGS = {
  ...
  overall_loseBy5: -1,
  attacking_cap: 2,
  attacking_cover: 0.333,
  attacking_assist: 1,
  defending_return: 0.2,
  defending_flagKill: 0.1,
  defending_seal: 1,
  extra_cleanSheet: 2,
  min_matches: 0,
...
function calculateMatchPoints(stat, redScore, blueScore, settings) {
  ...
  const coverPoints  = (stat.flag_cover  || 0) * settings.attacking_cover;
  const assistPoints = (stat.flag_assist  || 0) * settings.attacking_assist;
  const returnPoints   = (stat.flag_return || 0) * settings.defending_return;
  const flagKillPoints = (stat.flag_kill  || 0) * settings.defending_flagKill;
...
function getMatchBreakdown(stat, redScore, blueScore, settings) {
  ...
  return {
    ...
    cover_points:    (stat.flag_cover  || 0) * settings.attacking_cover,
    assist_points:   (stat.flag_assist  || 0) * settings.attacking_assist,
    return_points:   (stat.flag_return || 0) * settings.defending_return,
    flag_kill_points:(stat.flag_kill   || 0) * settings.defending_flagKill,
